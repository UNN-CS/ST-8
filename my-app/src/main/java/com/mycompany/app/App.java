...
public class App {
    ...
    private static Map<String, Object> readCDData() throws IOException {
        ...
        if (!data.containsKey("Artist") || !data.containsKey("Title") || tracks.isEmpty()) {
            throw new IOException("Файл data.txt содержит неполные или неверные данные");
        }
    ...
    private static void handleDownloadedPdf(Path resultDir) {
        try {
            ...
            try {
                ...
                System.out.println("Существующий cd.pdf удалён");
            } catch (IOException e) {
                System.err.println("Не удалось удалить старый cd.pdf: " + e.getMessage());
            }
